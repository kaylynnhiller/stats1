...
We did not select arr_delay in the select function so it is not available to arrange. Select keeps only the columns listed.
...
```
Count allows you to count the unique values of one or more variables similar to group_by and summarise. The sort options causes the results to be presented in descending order.

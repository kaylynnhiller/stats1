...
rows in. Since they perform separate functions, the order does not
matter.
...
\item
  ...
  I would expect the scheduled departure time (sched\_dep\_time) plus
  the delay (dep\_delay) to equal the actual departure time (dep\_time).
\end{enumerate}
...
We did not select that variable in the select function so it is not
...
similar to group\_by and summarise. The sort options causes the results
...
The first function calculated the means for the three combinations of
the y and z column values. The second function creates a new column
where these means are stored. The output of the first function is three
rows and five rows for the second function.

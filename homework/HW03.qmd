...
-   **MODEL C (compact):** the reference value is 37 for this PTSD test. We can center our responses around 37 since we want to see if on average scores were greater or less than this reference value. I centered the model using: Let $\Delta_i = \text{PTSD}_i - 37$.

    Let $\Delta_i = \text{PTSD}_i - 37$.\
    $$\Delta_i = 0 + \varepsilon_i$$
...
-   **Null hypothesis:** $H_0: b_0 = 0$, The null hypothesis is that soldiers will have an average score equal to 37.
...
On average, do people tend to agree or disagree that teenagers should have access to birth control?
...
-   **MODEL C (compact):** the reference value is 2 for this analysis. We can center our responses around 2 since we want to see if on average scores were greater or less than this reference value. I centered the model using: Let $\Delta_i = \text{BC}_i - 2$.

    Let $\Delta_i = \text{BC}_i - 2$.\
    $$\Delta_i = 0 + \varepsilon_i$$
...
-   **Null hypothesis:** $H_0: b0 = 0$, The null hypothesis is that, on average, respondents neither lean toward agreement nor disagreement relative to ‘agree’.
